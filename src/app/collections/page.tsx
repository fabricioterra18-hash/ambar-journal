...
import { getCollections, getCollectionItemCounts } from '@/lib/services/collections'
...
export default async function CollectionsPage() {
  ...
  const [collections, counts] = await Promise.all([
    ...
    getCollectionItemCounts(workspace.id),
  ])
    ...
    <main className="flex-1 flex flex-col p-5 pb-32">
      ...
      <div className="grid grid-cols-2 gap-3">
        {collections.map((collection, i) => {
          ...
          const itemCount = counts[collection.id] ?? 0
          return (
            <Link
              key={collection.id}
              href={`/collections/${collection.id}`}
              className={`rounded-2xl p-5 border ${COLORS[colorIdx]} hover:-translate-y-0.5 transition-all active:scale-98 block`}
            >
              <div className={`w-10 h-10 rounded-xl bg-surface/60 flex items-center justify-center mb-3 ${ICON_COLORS[colorIdx]}`}>
                <Folder size={20} />
              </div>
